...
def load_and_preprocess_data(data_file):
    ...
    for label in labels_to_sample:
        label_df = df_new[df_new['label'] == label]
        sampled_dfs.append(label_df.sample(frac=1))
    df_balanced = pd.concat(sampled_dfs)
    label_encoder = LabelEncoder()
    df_balanced['category'] = label_encoder.fit_transform(df_balanced['label'])
    return df_balanced['title'].tolist(), df_balanced['category'].tolist(), label_encoder
...
def train_and_evaluate(model, train_dataloader, val_dataloader, optimizer, scheduler, device):
    best_accuracy = 0
    for epoch in range(num_epochs):
        model.train()
        ...
        with torch.no_grad():
            ...
            for batch in val_dataloader:
                ...
                _, predicted_labels = torch.max(outputs, dim=1)
                total_eval_accuracy += (predicted_labels == labels).sum().item()
...
if __name__ == "__main__":
    ...
    best_accuracy = train_and_evaluate(model, train_dataloader, val_dataloader, optimizer, scheduler, device)
